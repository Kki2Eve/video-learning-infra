...
if not hasattr(mp, "solutions"):
    raise ImportError(
        "This project requires a MediaPipe build with `mp.solutions`.\n"
        "Please activate the `dexrt` env and install a compatible version, e.g.:\n"
        "  conda activate dexrt\n"
    ...
    def draw_skeleton_on_image(
        ...
        if style == "default":
            ...
            for pair in connections:
                connection_style[pair] = mp.solutions.drawing_utils.DrawingSpec(
                    thickness=2
                )
